...
def _try_json_loads_maybe(value: Any) -> Any:
    """Best-effort JSON parsing: if value is a JSON string, parse it; otherwise return as-is."""
...
def parse_varta_cgi_data(text: str) -> Any:
    """Parst die Antwort von /cgi/data.

    Je nach Firmware/Fehlerzustand kann das sein:
    - JSON-Objekt direkt
    - JSON-String, der wiederum JSON enthaelt (double-encoded)
    - Sonstige Strings/HTML (z.B. Login-Seite) -> dann wird ein ValueError geworfen
    """
    raw = (text or "").strip()
    if not raw:
        raise ValueError("Leere Antwort von /cgi/data")

    # Häufig: ein valider JSON-String
    try:
        obj: Any = json.loads(raw)
    except Exception as e:
        # z.B. HTML oder sonstiges
        snippet = raw[:200].replace("\n", " ")
        raise ValueError(f"Antwort von /cgi/data ist kein JSON (Beginn: {snippet!r})") from e

    # Manchmal ist es JSON-im-JSON (String, der wiederum JSON ist)
    obj2 = _try_json_loads_maybe(obj)
    # Manche Geräte liefern sogar mehrfach verschachtelt
    obj3 = _try_json_loads_maybe(obj2)
    return obj3


def _flatten_dict_to_metrics(obj: Dict[str, Any], prefix: str = "") -> Dict[str, Dict[str, Any]]:
    """Wandelt verschachtelte Dicts in Metric-Gruppen um.

    Beispiel:
      {"common": {"power_W": 1}, "pulse": {"procImg": {"soc_pct": 2}}}

    wird zu:
      {
        "common": {"power_W": 1},
        "pulse.procImg": {"soc_pct": 2},
        ...
      }

    Nur Leafs, die keine Dicts sind, werden als Messwerte behandelt. Listen werden als Wert durchgereicht.
    """
    out: Dict[str, Dict[str, Any]] = {}

    def walk(d: Dict[str, Any], path_parts: List[str]):
        ...
        if leaf_values:
            metric_name = ".".join([p for p in ([prefix] + path_parts) if p])
            out.setdefault(metric_name, {}).update(leaf_values)

    walk(obj, [])
    return out


def normalize_final(obj: Any) -> Dict[str, List[Dict[str, Any]]]:
    """Normalisiert die Varta Payload auf: Dict[str, List[Dict[str, Any]]].

    Unterstützt zwei Formen:
    1) Alt: {"Inverter": [{"EMS PExtra": 0}, ...], ...}
    2) Neu/verschachtelt: {"common": {...}, "pulse": {"procImg": {...}}}
       -> wird zu Metrik-Gruppen: "common", "pulse.procImg", ...

    - Pro Metric muss eine Liste existieren.
    - Einträge in der Liste müssen Dicts sein.
    - String-Einträge werden best-effort als JSON geparst; wenn das kein Dict ergibt, werden sie verworfen.
    """
    obj = _try_json_loads_maybe(obj)

    if obj is None:
        return {}

    if not isinstance(obj, dict):
        # Unbekanntes Format -> nichts exportieren statt crashen
        return {}

    # Wenn das Top-Level wie die neue Payload aussieht (verschachtelte dicts), flachziehen.
    # Heuristik: sobald mindestens ein Top-Level-Wert ein dict ist und kein Top-Level-Wert eine list ist.
    if any(isinstance(v, dict) for v in obj.values()) and not any(isinstance(v, list) for v in obj.values()):
        flattened = _flatten_dict_to_metrics(obj)
        obj = {metric: [values] for metric, values in flattened.items()}

    normalized: Dict[str, List[Dict[str, Any]]] = {}
    for metric, entries in obj.items():
        if isinstance(entries, dict):
            # selten: metric -> dict statt list von dicts
            entries_list: List[Any] = [entries]
        elif isinstance(entries, list):
            entries_list = entries
        else:
            # z.B. metric -> string/number/bool: als 1-element dict ablegen
            normalized[str(metric)] = [{"value": entries}]
            continue

        out_entries: List[Dict[str, Any]] = []
        for e in entries_list:
            e = _try_json_loads_maybe(e)
        ...
        normalized[str(metric)] = out_entries
...
def main(host: str,
    ...
    def _check_logged_in():
        ...
        try:
            ...
            if e.response.status_code == 403 or e.response.status_code == 401:
                # We are not logged in, so we need to log in
                return log_in()

        try:
            results = response.json()
        except Exception as e:
            print(f"Fehler beim Parsen der User Check Antwort: {e}")
            # Wrong Level, so we need to log in
            return log_in()

        # Check if "access" is 3, which indicates already logged in
        if "access" not in results or results["access"] != 3:
            return log_in()
        else:
            return True
    ...
    def log_in():
        ...
        login_data = {"username": varta_user, "password": varta_password}
    ...
    while True:
        ...
        try:
            final_raw = parse_varta_cgi_data(data)
